CREATE USER omesa;
ALTER USER omesa WITH PASSWORD 'Colombia2021';

CREATE TABLESPACE curso_backend_2021 LOCATION 'C:\Users\W1803\Documents\Proyecto\Agil\Curso de BackEnd\proyectos\DATA';

CREATE DATABASE academia WITH 
OWNER = 'omesa'
ENCODING = 'UTF8'
TABLESPACE = curso_backend_2021;

CREATE DATABASE academia WITH 
ENCODING = 'UTF8';

CREATE SCHEMA colegio1;
CREATE SCHEMA colegio2;
...
INSERT INTO tipo_actores VALUES (NEXTVAL ('tipo_actores_seq'), 'Estudiante');
INSERT INTO tipo_actores VALUES (NEXTVAL ('tipo_actores_seq'), 'Docente');
...
ALTER TABLE actores ADD CONSTRAINT fk_estado_actor FOREIGN KEY (estado_actor_id) REFERENCES estado_actores (id) ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE actores ADD CONSTRAINT fk_institucion_estudiante FOREIGN KEY (institucion_id) REFERENCES instituciones_educativas (id) ON DELETE RESTRICT ON UPDATE CASCADE;
ALTER TABLE actores ADD CONSTRAINT fk_tipo_documento FOREIGN KEY (tipo_documento) REFERENCES tipo_documento (codigo) ON DELETE RESTRICT ON UPDATE CASCADE;
...
COMMENT ON CONSTRAINT uq_ie_grado ON grupos IS 'Un grado, de una institucion educativa solo puede estar presente una sola vez en un año	0	rnUn grado, de una institucion educativa solo puede estar presente una sola vez en un año';
